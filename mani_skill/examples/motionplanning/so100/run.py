...
def parse_args(args=None):
    ...
    parser.add_argument("--robot-uid", type=str, default="so100", help="The robot to use. Robot setups supported for teleop in this script is so100")
    ...
    while True:
        ...
        if args.only_count_success and not success:
            ...
            if passed == args.num_traj:
                break
    # Proper cleanup to prevent segfault
    try:
        env.close()
        del env
    except:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Force cleanup to prevent segfault
        import gc
        gc.collect()
        import time
        time.sleep(0.1)  # Brief pause before exit
